...
public class QuestionsPanel {
    ...
    public void timeForAnswer() {
        Timeline timeline = new Timeline(
                ...
                new KeyFrame(Duration.seconds(10), e -> {
                    try {
                        System.out.println("Player Round Total: " + PointCount.playerRoundTotal(round));

                        System.out.println( "Player Game Total: " +PointCount.playerGameTotal());

                        // TODO: 18/11/2020 somehow get these into ResultAndReview.fxml
    ...
    public void answerOn(ActionEvent actionEvent) {
        ...
        if (buttonCLicked.getText().equals(model.getQ().getRightAnswer())) {
            ...
            if(questionNBR < 3){
                PointCount.playerPointCount(1,round,questionNBR); // adds 1 point for answering the question correctly
            ...
            if(questionNBR < 3) {
                PointCount.playerPointCount(0, round, questionNBR); // adds 0 points for answering the question wrong

                questionNBR++;
            } else{
                questionNBR = 0; //Resets teh questionNBR
            }
        }
    }
